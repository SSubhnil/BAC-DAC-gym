...
class mountain_car_continuous_v0:
    def __init__(self, gym_env, **kwargs):#Initialize Domain Parameters
        ...
        self.POS_MAP_RANGE = cp.array([[0],[1]])
        self.VEL_MAP_RANGE = cp.array([[0],[1]])
        self.GRID_SIZE = cp.array([[4], [4]])
        ...
        self.SIG_GRID = self.sig_grid2 * cp.identity(2)
        self.INV_SIG_GRID = cp.linalg.inv(self.SIG_GRID)
        self.phi_x = cp.zeros((self.NUM_STATE_FEATURES, 1))
        self.NUM_ACT = 2 ## -1.0 and 1.0 since Continuous control problem
        self.ACT = cp.array([action_space.low[0], action_space.high[0]])
        self.num_policy_param = self.NUM_STATE_FEATURES * self.NUM_ACT
        self.STEP = 1
        
        self.prng = cp.random.RandomState()
        self.prng.seed(2)
    ...
    def calc_score(self, theta, state):
        ...
        #feature values
        phi_x = cp.zeros((self.NUM_STATE_FEATURES, 1))
        mu = cp.zeros(self.NUM_ACT)
        tmp1 = cp.zeros((2, 1))
        ...
        for tt in range(0, self.NUM_STATE_FEATURES):
            ...
            arbi1 = cp.dot(self.INV_SIG_GRID, tmp1)
            phi_x[tt, 0] = cp.exp(-0.5 * cp.dot(cp.transpose(tmp1), arbi1)).item()
            
        for tt in range(0, self.NUM_ACT):
            if tt == 0:
                phi_xa = cp.vstack((phi_x, cp.zeros((self.NUM_STATE_FEATURES, 1)))) 
                
            else:
                phi_xa = cp.vstack((cp.zeros((self.NUM_STATE_FEATURES, 1)), phi_x))

            lol = cp.dot(cp.transpose(phi_xa), theta)
                
            mu[tt] = cp.exp(lol.item())
        ...
        if tmp2 < mu[0]:
            a = self.ACT[0] #* tmp2
            scr = cp.vstack((phi_x * (1 - mu[0]),
                            -phi_x * mu[1]))
    
        else:
            a = self.ACT[-1] #* tmp2
            scr = cp.vstack((-phi_x * mu[0],
    ...
    def kernel_kx(self, state, statedic, domain_params):
        ...
        xdic = cp.hstack(xdic)
        arbitrary = cp.vstack([self.c_map_pos[0][0], self.c_map_vel[0][0]])
        y = cp.multiply(arbitrary, x)### We will see
        ydic = cp.multiply(mb.repmat(arbitrary, 1, cp.shape(xdic)[1]),  xdic)
        # Element-wise squaring of Euclidean pair-wise distance
        #Need to install pdist python package 
        temp = distance.cdist(cp.transpose(y), cp.transpose(ydic)) ** 2
        kx = ck_x * cp.exp((-1 * temp) / (2 * sigk_x*sigk_x))
        return cp.squeeze(kx)
            ...
            while done == 0 or t < learning_params.episode_len_max:
                for istep in range(self.STEP):
                    if done == 0:
                        ...
                        state[0], reward, done, _ = self.gym_env.step(cp.array([a])) ### Fix this array methods
    ...
    def dynamics(self, state, a_old, domain_params):
        ...
        y = cp.array([[(self.c_map_pos[0] * x[0]) + self.c_map_pos[1]],
